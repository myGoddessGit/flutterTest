...
class _IndexBarState extends State<_IndexBar>{
  ...
  ///构建tag
  Widget _buildIndexBarTagWidget(BuildContext context, String tag, IndexBarDetails indexModel){
  ...
  /// 构建indexBar hint
  Widget _buildIndexBarHintWidget(BuildContext context, String tag, IndexBarDetails indexModel){
    // 如果外界自定义 indexbarHint
    if (widget.indexBarHintBuilder != null) {
      return widget.indexBarHintBuilder(context, tag, indexModel);
    } else {
      // 图片的路径
      final String hintImagePath = widget.hintImagePath ?? null;// null为自己的图片
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.tag != null && widget.tag.isNotEmpty && widget.tag != _indexModel.tag){
